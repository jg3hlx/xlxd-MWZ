...
//  Based on NXDN2DMR by Jonathan Naylor G4KLX and Andy Uribe CA6JAU
//
// ----------------------------------------------------------------------------
...
const uint8 BIT_MASK_TABLE[] = {0x80U, 0x40U, 0x20U, 0x10U, 0x08U, 0x04U, 0x02U, 0x01U};

#define WRITE_BIT(p,i,b) p[(i)>>3] = (b) ? (p[(i)>>3] | BIT_MASK_TABLE[(i)&7]) : (p[(i)>>3] & ~BIT_MASK_TABLE[(i)&7])
#define READ_BIT(p,i)    (p[(i)>>3] & BIT_MASK_TABLE[(i)&7])

////////////////////////////////////////////////////////////////////////////////////////
// PRNG table for NXDN AMBE scrambling
...
////////////////////////////////////////////////////////////////////////////////////////
// Bit position tables for AMBE interleaving
...
const uint8 CNxdnUtils::AMBE_SILENCE[9] = {0xB9U, 0xE8U, 0x81U, 0x52U, 0x61U, 0x73U, 0x00U, 0x2AU, 0x6BU};

////////////////////////////////////////////////////////////////////////////////////////
// Decode 4 AMBE frames from NXDN voice payload

void CNxdnUtils::DecodeVoice(const uint8 *payload, uint8 **ambe)
{
    // Skip LICH (1 byte) and SACCH (4 bytes) = 5 bytes
    const uint8 *data = payload + 5U;

    // Voice block 1: 2 AMBE frames at bit offsets 0 and 49
    DecodeAmbe(data, ambe[0], 0U);
    DecodeAmbe(data, ambe[1], 49U);

    // Voice block 2: 2 AMBE frames at bit offsets 0 and 49
    data += 14U;
    DecodeAmbe(data, ambe[2], 0U);
    DecodeAmbe(data, ambe[3], 49U);
}

////////////////////////////////////////////////////////////////////////////////////////
// Decode single AMBE frame from NXDN voice block
// This extracts 49 bits from NXDN interleaved format and produces 9-byte AMBE frame

void CNxdnUtils::DecodeAmbe(const uint8 *in, uint8 *out, unsigned int offset)
{
    // Clear output
    ::memset(out, 0x00U, 9U);
    ...
    // Extract a bits (12 bits) and b bits (12 bits) from raw data
    ...
    // Extract c bits (25 bits) from raw data
    MASK = 0x1000000U;
    ...
    // Write b bits to output using interleaving table
    MASK = 0x400000U;
    ...
    // Write c bits to output using interleaving table
    MASK = 0x1000000U;
    for (unsigned int i = 0U; i < 25U; i++, MASK >>= 1) {
...
////////////////////////////////////////////////////////////////////////////////////////
// Encode single AMBE frame into NXDN voice block
// This takes 9-byte AMBE frame and produces 49 bits in NXDN interleaved format

void CNxdnUtils::EncodeAmbe(const uint8 *in, uint8 *out, unsigned int offset)
{
    // Extract a bits from interleaved AMBE data
    unsigned int a = 0U;
    ...
    // Extract b bits from interleaved AMBE data
    unsigned int b = 0U;
    MASK = 0x400000U;
    for (unsigned int i = 0U; i < 23U; i++, MASK >>= 1) {
        unsigned int bPos = B_TABLE[i];
    ...
    // Extract c bits from interleaved AMBE data
    unsigned int c = 0U;
    MASK = 0x1000000U;
    for (unsigned int i = 0U; i < 25U; i++, MASK >>= 1) {
        unsigned int cPos = C_TABLE[i];
    ...
    // Golay decode a bits (top 12 of 24-bit Golay code)
    unsigned int aDecoded = a >> 12;

    // PRNG descramble b bits (same as YSF VD2Vch)
    unsigned int bDescrambled = b ^ (PRNG_TABLE[aDecoded] >> 1);
    unsigned int bDecoded = bDescrambled >> 11;

    // Write raw bits to output at offset
    MASK = 0x000800U;
    for (unsigned int i = 0U; i < 12U; i++, MASK >>= 1) {
        unsigned int aPos = i + offset + 0U;
        unsigned int bPos = i + offset + 12U;
        WRITE_BIT(out, aPos, aDecoded & MASK);
        WRITE_BIT(out, bPos, bDecoded & MASK);
    }
...
////////////////////////////////////////////////////////////////////////////////////////
// Encode 4 AMBE frames into NXDN voice payload with proper SACCH

void CNxdnUtils::EncodeVoice(uint8 **ambe, uint8 *payload, uint16_t srcId, uint16_t dstId, uint8_t packetNum)
{
    // Clear payload
    ::memset(payload, 0x00U, 33U);

    // Set LICH byte for voice frame (byte 0)
    // NXDN receivers expect valid LICH to process voice
    payload[0] = 0xACU;  // NXDN_LICH_VOICE

    // Build Layer3 VCALL message (72 bits = 9 bytes)
    // Structure:
    //   Byte 0: Message type (bits 0-5 = 0x01 for VCALL)
    //   Byte 1: Call options (0x00)
    //   Byte 2: Cipher info (bit 7 = 0 for group call)
    //   Bytes 3-4: Source ID (big endian)
    //   Bytes 5-6: Destination ID (big endian)
    //   Bytes 7-8: Reserved
    uint8_t layer3[9];
    ::memset(layer3, 0x00U, 9U);
    layer3[0] = 0x01U;                      // VCALL message type
    layer3[1] = 0x00U;                      // Call options
    layer3[2] = 0x20U;                      // Group call (bit 5 = 1)
    layer3[3] = (srcId >> 8) & 0xFFU;       // Source ID high byte
    layer3[4] = srcId & 0xFFU;              // Source ID low byte
    layer3[5] = (dstId >> 8) & 0xFFU;       // Dest ID high byte
    layer3[6] = dstId & 0xFFU;              // Dest ID low byte

    // SACCH structure cycles through 3,2,1,0 (SR_1_4, SR_2_4, SR_3_4, SR_4_4)
    // Each structure carries 18 bits of Layer3 data
    uint8_t structure = 3U - (packetNum % 4U);
    uint8_t sacchOffset = (packetNum % 4U) * 18U;

    // SACCH byte 0: Structure (bits 6-7) + RAN (bits 0-5, use 0)
    payload[1] = (structure << 6) & 0xC0U;

    // Extract 18 bits from Layer3 starting at sacchOffset and pack into SACCH bytes 1-3
    // Using bit manipulation to extract and pack
    for (unsigned int i = 0U; i < 18U; i++) {
        unsigned int srcBit = sacchOffset + i;
        unsigned int srcByte = srcBit / 8U;
        unsigned int srcBitPos = 7U - (srcBit % 8U);

        unsigned int dstBit = i + 8U;  // Start at bit 8 of SACCH (after structure+RAN)
        unsigned int dstByte = dstBit / 8U;
        unsigned int dstBitPos = 7U - (dstBit % 8U);

        if (layer3[srcByte] & (1U << srcBitPos)) {
            payload[1 + dstByte] |= (1U << dstBitPos);
        }
    }

    // Compute and append SACCH CRC-6 over the 26-bit SACCH payload
    // (structure + RAN + 18 bits Layer3 data) → bits 26-31
    EncodeSacchCRC6(payload + 1U);

    // Voice data starts at byte 5
    uint8 *data = payload + 5U;

    // Voice block 1: 2 AMBE frames at bit offsets 0 and 49
    EncodeAmbe(ambe[0], data, 0U);
    EncodeAmbe(ambe[1], data, 49U);

    // Voice block 2: 2 AMBE frames at bit offsets 0 and 49
    data += 14U;
    EncodeAmbe(ambe[2], data, 0U);
    EncodeAmbe(ambe[3], data, 49U);
}

////////////////////////////////////////////////////////////////////////////////////////
// TX_REL (Transmission Release) encoding

// Bit manipulation helpers (matching G4KLX MMDVMHost conventions)
static const uint8 NXDN_BIT_MASK[] = { 0x80U, 0x40U, 0x20U, 0x10U, 0x08U, 0x04U, 0x02U, 0x01U };

#define NXDN_READ_BIT(p,i)    ((p)[(i)>>3] & NXDN_BIT_MASK[(i)&7])
#define NXDN_WRITE_BIT(p,i,b) (p)[(i)>>3] = (b) ? ((p)[(i)>>3] | NXDN_BIT_MASK[(i)&7]) : ((p)[(i)>>3] & ~NXDN_BIT_MASK[(i)&7])

uint16_t CNxdnUtils::ComputeCRC12(const uint8 *in, unsigned int lengthBits)
{
    uint16_t crc = 0x0FFFU;

    for (unsigned int i = 0U; i < lengthBits; i++)
    {
        bool bit1 = NXDN_READ_BIT(in, i) != 0x00U;
...
void CNxdnUtils::EncodeCRC12(uint8 *in, unsigned int lengthBits)
{
    uint16_t crc = ComputeCRC12(in, lengthBits);

    uint8 temp[2U];
    temp[0U] = (crc >> 8) & 0xFFU;
    temp[1U] = (crc >> 0) & 0xFFU;

    unsigned int n = lengthBits;
    for (unsigned int i = 4U; i < 16U; i++, n++)
    {
        bool b = NXDN_READ_BIT(temp, i) != 0x00U;
        NXDN_WRITE_BIT(in, n, b);
    }
}

void CNxdnUtils::EncodeSacchCRC6(uint8 *sacch)
{
    // Compute CRC-6 over first 26 bits of 4-byte SACCH raw data
    // (structure + RAN + 18 bits Layer3 data)
    // Polynomial: 0x27, init: 0x3F (matches G4KLX NXDNCRC::createCRC6)
    uint8_t crc = 0x3FU;

    for (unsigned int i = 0U; i < 26U; i++)
    {
        bool bit1 = NXDN_READ_BIT(sacch, i) != 0x00U;
    ...
    // Write 6-bit CRC into bits 26-31 of the SACCH
    uint8_t temp[1U];
    temp[0U] = crc;
    unsigned int n = 26U;
    ...
    {
        bool b = NXDN_READ_BIT(temp, i) != 0x00U;
        NXDN_WRITE_BIT(sacch, n, b);
    }
}

void CNxdnUtils::EncodeVcallHeader(uint8 *payload, uint16_t srcId, uint16_t dstId)
{
    EncodeFacch1Payload(payload, 0x01U, srcId, dstId);  // VCALL = 0x01
}

void CNxdnUtils::EncodeTxRel(uint8 *payload, uint16_t srcId, uint16_t dstId)
{
    EncodeFacch1Payload(payload, 0x08U, srcId, dstId);  // TX_REL = 0x08
}

void CNxdnUtils::EncodeFacch1Payload(uint8 *payload, uint8_t messageType, uint16_t srcId, uint16_t dstId)
{
    // Clear payload
    ::memset(payload, 0x00U, 33U);

    // Byte 0: LICH raw byte
    // RFCT=RDCH(2), FCT=SACCH_NS(0), Option=STEAL_FACCH(0), Direction=INBOUND(0), Parity=1
    // Binary: 10 00 00 0 1 = 0x81
    payload[0] = 0x81U;

    // Bytes 1-4: SACCH (idle, structure=SINGLE)
    // Byte 1 bits 7-6 = structure (0 = SINGLE), bits 5-0 = RAN (0)
    // Bytes 2-4: idle data (zeros) + CRC-6
    payload[1] = 0x00U;
    EncodeSacchCRC6(payload + 1U);

    // Bytes 5-16: Raw FACCH1 data (10 bytes Layer3 + 2 bytes CRC12)
    // Layer3 message format:
    //   Byte 0: Message type in lower 6 bits (0x01=VCALL, 0x08=TX_REL)
    //   Byte 1: Call options = 0x00
    //   Byte 2: Cipher info, bit 7 = 0 for group call
    //   Bytes 3-4: Source ID (big endian)
    //   Bytes 5-6: Destination ID (big endian)
    //   Bytes 7-9: Reserved (zeros)
    uint8 *facch1 = payload + 5U;
    facch1[0] = messageType;                // Layer3 message type
    facch1[1] = 0x00U;                      // Call options
    facch1[2] = 0x20U;                      // Group call (bit 5 = 1, required by Icom/OpenSpot firmware)
    facch1[3] = (srcId >> 8) & 0xFFU;       // Source ID high byte
    facch1[4] = srcId & 0xFFU;              // Source ID low byte
    facch1[5] = (dstId >> 8) & 0xFFU;       // Dest ID high byte
    facch1[6] = dstId & 0xFFU;              // Dest ID low byte
    // Bytes 7-9: zeros (already cleared)

    // Compute and append CRC-12 over the first 80 bits (10 bytes)
    EncodeCRC12(facch1, 80U);

    // Bytes 19-30: Duplicate FACCH1 raw data in block 2
    // (Both voice blocks are "stolen" for FACCH1)
    ::memcpy(payload + 19U, facch1, 12U);
}
