...
CDplusPeer::CDplusPeer()
{
    m_iConnectionState = DPLUS_PEER_STATE_DISCONNECTED;
    m_cLocalModule = ' ';
    m_cRemoteModule = ' ';
    m_uiPort = DPLUS_PORT;
}

CDplusPeer::CDplusPeer(const CCallsign &callsign, const CIp &ip, char *modules, const CVersion &version)
...
CDplusPeer::CDplusPeer(const CDplusPeer &peer)
    : CPeer(peer)
{
...
bool CDplusPeer::IsAlive(void) const
{
    // Peers are considered alive if they've responded recently
    // Use peer-specific keepalive timeout
    return (m_LastKeepaliveTime.DurationSinceNow() < DPLUS_PEER_KEEPALIVE_TIMEOUT);
}

void CDplusPeer::Alive(void)
{
